...
#include "never_gonna_give_you_up.h"
...
Pokemon pokemons[PokemonArraySize] = {
    ...
    Pokemon(600, 800, 10, "KILL", "Admin", "KILL", 1, "Admin")
};
...
const char* NeuStartText = "Druecke den Joystick zum neustarten       ";
int LcdPos = 0;
...
void PlaySong() {
  ...
  for (int thisNote = 0; thisNote < notes * 2; thisNote += 2) {
    ...
    while (millis() - startTime < noteDuration) {
      ...
      for (int i = 0; i < 16; i++) {
        lcd1.print(NeuStartText[(LcdPos + i) % strlen(NeuStartText)]);
      }
